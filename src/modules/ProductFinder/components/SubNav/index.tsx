...
export const SubNav : FC<any> = ({ actionItemsLeft, actionItemsRight, filterHook, tabState, handleSortBy, handleDirectionSwitching, filtersList, direction, sortBy, openFilterModal, saveFilter }) => {
    ...
    return <Container maxWidth={"xl"}>
            <Box sx={(theme: any) => ({
                ...
                <Box style={{flex: 1, display: "flex", gap: "1rem"}}>
                    {actionItemsLeft.map((actionItem: any, index: number) => <React.Fragment key={actionItem.title + " " + index}>{actionItem(stuck)}</React.Fragment>)}
                </Box>
                ...
                <Box sx={{flex: 1, justifySelf: "flex-end", display: "flex", gap: "1rem", justifyContent: "flex-end"}}>
                    {actionItemsRight.map((actionItem: any, index: number) => <React.Fragment key={actionItem.title + " " + index}>{actionItem(stuck)}</React.Fragment>)}
                </Box>
            </Box>
        </Container>
}