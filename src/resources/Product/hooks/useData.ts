...
import {GET_PARTNERPROGRAMS, DELETE_PARTNERPROGRAM, CREATE_PARTNERPROGRAM, UPDATE_PARTNERPROGRAM } from "@schemas/partnerProgram";
...
export const useData = (meta = {direction: 1, sortBy: "name", limit: 10, filters: []}) => {
    ...
    const {refetch, loading, error, data, called, networkStatus} = useQuery(GET_PARTNERPROGRAMS, {
        variables: {
            meta: {
                ...meta,
                ...{direction: order === "asc" ? 1 : -1, sortBy: orderBy, limit: rowsPerPage, filters: [] },
    ...
    const [deleteMutation, {deleteErrors}] = useMutation(DELETE_PARTNERPROGRAM, {
    ...
    useEffect(() => {
        ...
        setItems([]);
        if (data) {
            setTotal(data.getPartnerPrograms.pageInfo.total);
            setItems(data.getPartnerPrograms.items);
        }
    }, [data]);
    ...
    return {
        ...
        setRowsPerPage,
        deleteItems,
        createMutation: CREATE_PARTNERPROGRAM,
        editMutation: UPDATE_PARTNERPROGRAM,
        refetchingOptions
    }
}
