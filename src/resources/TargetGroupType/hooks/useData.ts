...
import {DELETE_TARGETGROUPTYPE, GET_TARGETGROUPTYPES, CREATE_TARGETGROUPTYPE, UPDATE_TARGETGROUPTYPE } from "@schemas/targetGroupType";
...
export const useData = (meta = {direction: 1, sortBy: "name", limit: 10, filters: []}) => {
    ...
    const {refetch, loading, error, data, called, networkStatus} = useQuery(GET_TARGETGROUPTYPES, {
        variables: {
            meta: {
                ...meta,
                ...{direction: order === "asc" ? 1 : -1, sortBy: orderBy, limit: rowsPerPage, filters: [] },
    ...
    const [deleteMutation, {deleteErrors}] = useMutation(DELETE_TARGETGROUPTYPE, {
    ...
    useEffect(() => {
        ...
        setItems([]);
        if (data) {
            setTotal(data.getTargetGroupTypes.pageInfo.total);
            setItems(data.getTargetGroupTypes.items);
        }
    }, [data]);
    ...
    return {
        ...
        searchValue,
        setSearchValue,
        emptyData: !(!!data?.getTargetGroupTypes?.items?.length),
        order,
        orderBy,
        setOrderBy,
        setOrder,
        ...
        setRowsPerPage,
        deleteItems,
        createMutation: CREATE_TARGETGROUPTYPE,
        editMutation: UPDATE_TARGETGROUPTYPE,
        refetchingOptions
    }
}
